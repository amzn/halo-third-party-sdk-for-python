...
from halo_third_party_sdk_events.verifier_constants import (
    SIGNATURE_CERT_CHAIN_URL_HEADER, SIGNATURE_HEADER,
    CERT_CHAIN_DOMAINS, CHARACTER_ENCODING, MAX_NORMAL_REQUEST_TOLERANCE_IN_MILLIS)
...
class TestRequestVerifier(unittest.TestCase):
    PREPOPULATED_CERT_URL = "https://s3.amazonaws.com/healthtech.api/healthtech-api-cert-v000004.pem"
    VALID_URLS = [
    ...
    VALID_URL_WITH_PORT = "https://s3.amazonaws.com:443/healthtech.api/cert"
    VALID_URL_WITH_PATH_TRAVERSAL = (
        "https://s3.amazonaws.com/healthtech.api/../healthtech.api/cert")
    INVALID_URL_WITH_INVALID_HOST_NAME = "https://very.bad/healthtech.api/cert"
    ...
    def create_self_signed_certificate(self, cert_chain_domain):
        ...
        subject = issuer = x509.Name([
            ...
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, u"Amazon Halo"),
            x509.NameAttribute(
    ...
    def load_valid_certificate(self):
        with open(os.path.join(
                ...
                'data',
                'healthtech-api-cert-v000004.pem'), 'rb') as cert_response:
    ...
    def test_no_cert_url_header_throw_exception(self):
        ...
        self.assertIn(
            "Missing Signature/Certificate for the event",
            str(exc.exception))
    ...
    def test_validate_cert_url_for_valid_url(self):
        try:
            for url in self.VALID_URLS:
                self.request_verifier._validate_certificate_url(url)
        except:
    ...
    def test_load_cert_chain_invalid_cert_url_throw_exception(self):
        ...
        with mock.patch(
                "halo_third_party_sdk_events.verifier.urlparse",
    ...
    def test_load_cert_chain_load_and_cache_cert(self):
        self.assertIsNone(
            ...
            "Cache")
        with mock.patch(
                "halo_third_party_sdk_events.verifier.urlopen", autospec=True
        ) as mock_url_open:
            if six.PY2:
    ...
    def test_valid_end_cert_domains(self):
        for domain in CERT_CHAIN_DOMAINS:
            self.create_self_signed_certificate(domain)
            try:
    ...
    def test_invalid_end_cert_domain(self):
        self.create_self_signed_certificate(CERT_CHAIN_DOMAINS[0] + "asdf")
        with self.assertRaises(VerificationException) as exc:
            self.request_verifier._validate_end_certificate(self.mock_certificate)

    def test_validate_end_cert_valid_cert(self):
        self.create_self_signed_certificate(CERT_CHAIN_DOMAINS[0])
        try:
            self.request_verifier._validate_end_certificate(self.mock_certificate)
        except:
            # Should never reach here
...
class TestTimestampVerifier(unittest.TestCase):
    ...
    def test_timestamp_verification_with_expired_timestamp(self):
        test_request_envelope = RequestEnvelope(timestamp="2019-01-01T00:00:00Z")
        self.timestamp_verifier = TimestampVerifier()
        with self.assertRaises(VerificationException) as exc:
    ...
    def test_timestamp_verification_with_valid_future_server_timestamp(self):
        ...
        valid_future_datetime = (datetime.now(tzutc()) + timedelta(seconds=valid_tolerance)).isoformat()
        test_request_envelope = RequestEnvelope(timestamp=valid_future_datetime)
        self.timestamp_verifier = TimestampVerifier()
        try:
            self.timestamp_verifier.verify(
                headers={},
    ...
    def test_timestamp_verification_with_valid_timestamp(self):
        test_request_envelope = RequestEnvelope(timestamp=datetime.utcnow().isoformat()[:-3] + 'Z')
        self.timestamp_verifier = TimestampVerifier()
        try:
            self.timestamp_verifier.verify(
                headers={},
    ...
    def test_timestamp_verification_with_valid_timestamp_event(self):
        test_request_envelope = RequestEnvelope(timestamp="2020-01-01T00:59:00Z")
        self.timestamp_verifier = TimestampVerifier()
        try:
            self.timestamp_verifier.verify(
                headers={},
    ...
    def test_timestamp_verification_with_expired_timestamp_event(self):
        test_request_envelope = RequestEnvelope(timestamp="2020-01-01T00:00:00Z")
        self.timestamp_verifier = TimestampVerifier()
        with self.assertRaises(VerificationException) as exc:
            self.timestamp_verifier.verify(
                headers={},
    ...
    @freeze_time('2020-01-01 01:01:00')
    def test_timestamp_verification_with_custom_tolerance_and_expired_timestamp_event(self):
        one_min_in_milli = 60000
        test_request_envelope = RequestEnvelope(timestamp="2020-01-01T00:59:00Z")
        self.timestamp_verifier = TimestampVerifier(one_min_in_milli)
        with self.assertRaises(VerificationException) as exc:
            self.timestamp_verifier.verify(
                headers={},
                serialized_request_env="",
