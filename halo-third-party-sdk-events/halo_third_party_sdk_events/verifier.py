...
from six.moves.urllib.request import urlopen # nosec
...
from .verifier_constants import (
    ...
    CERT_CHAIN_URL_PORT, CERT_CHAIN_URL_STARTPATHS,
    CERT_CHAIN_DOMAINS, CHARACTER_ENCODING,
...
class AbstractVerifier(object):
    ...
    def verify(
        ...
        :type deserialized_request_env:
            :py:class:`halo_third_party_sdk_model.request_envelope.RequestEnvelope`
...
class RequestVerifier(AbstractVerifier):
    ...
    :py:mod:`halo_third_party_sdk_events.verifier_constant`.
    ...
    def __init__(
        ...
        :py:mod:`halo_third_party_sdk_events.verifier_constant`. Additionally,
        one can also provide the Padding and the Hash Algorithm functions
    ...
    def _validate_certificate_url(self, cert_url):
        ...
        if all(not normalized_path.startswith(path) for path in CERT_CHAIN_URL_STARTPATHS):
            raise VerificationException(
                "Signature Certificate URL has invalid path: {}. "
                "Expecting the path to start with {}".format(
                    normalized_path, CERT_CHAIN_URL_STARTPATHS))
    ...
    def _validate_end_certificate(self, x509_cert):
        ...
        - The end certificate contains Halo domain in it's SAN extensions
        ...
        if all(domain not in ext_value.get_values_for_type(DNSName) for domain in CERT_CHAIN_DOMAINS):
            raise VerificationException(
...
class TimestampVerifier(AbstractVerifier):
    ...
    def verify(
        ...
        if (deserialized_request_env.timestamp is None):
            raise VerificationException("Timestamp verification failed")
        else:
            request_timestamp = parser.isoparse(deserialized_request_env.timestamp)
